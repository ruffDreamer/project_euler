...
for i in range(3,20):
	for j in range(20):
		up = u[i][j]*u[i-1][j]*u[i-2][j]*u[i-3][j]
		if up > ph1:
			ph1 = up
...
for i in range(17):
	for j in range(20):
		down = u[i][j]*u[i+1][j]*u[i+2][j]*u[i+3][j]
		if down > ph1:
			ph1 = down

for i in range(20):
	for j in range(3,20):
		left = u[i][j]*u[i][j-1]*u[i][j-2]*u[i][j-3]
		if left> ph1:
			ph1 = left

for i in range(20):
	for j in range(17):
		right = u[i][j]*u[i][j+1]*u[i][j+2]*u[i][j+3]
		if right > ph1:
			ph1 = right

for i in range(17):
	for j in range(17):
		rdiagonal = u[i][j]*u[i+1][j+1]*u[i+2][j+2]*u[i+3][j+3]
		if rdiagonal  > ph1:
			ph1 = rdiagonal

for i in range(17):
	for j in range(3,20):
		ldiagonal = u[i][j]*u[i+1][j-1]*u[i+2][j-2]*u[i+3][j-3]		
		if ldiagonal > ph1:
			ph1 = ldiagonal
